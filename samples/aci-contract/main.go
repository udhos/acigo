...
func main() {
	...
	list, errList := a.ContractList(tenant)
	if errList != nil {
	...
	for _, t := range list {
		...
		log.Printf("FOUND contract: name=%s dn=%s scope=%s descr=%s", name, dn, scope, descr)
	}
}
...
func execute(a *aci.Client, cmd string, args []string) {
	...
	case "add":
		if len(args) < 3 {
			log.Fatalf("usage: %s add tenant contract scope [descr]", os.Args[0])
		}
		tenant := args[0]
		contract := args[1]
		scope := args[2]
		...
		errAdd := a.ContractAdd(tenant, contract, scope, descr)
		if errAdd != nil {
			log.Printf("FAILURE: add error: %v", errAdd)
			return
		}
		log.Printf("SUCCESS: add: %s %s %s %s", tenant, contract, scope, descr)
	case "del":
		if len(args) < 2 {
			log.Fatalf("usage: %s del tenant contract", os.Args[0])
		}
		tenant := args[0]
		contract := args[1]
		errDel := a.ContractDel(tenant, contract)
		if errDel != nil {
			log.Printf("FAILURE: del error: %v", errDel)
			return
		}
		log.Printf("SUCCESS: del: %s %s", tenant, contract)
	case "list":
	default:
